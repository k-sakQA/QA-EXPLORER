...
```
/explore url=<テスト対象のURL> charter=qa-knowledge/targets/<target-slug>/test-charters.md
```
承認済みのチャーターの**範囲・深さ・実行順だけ**を実行します。観点リストの優先度
（P0→P3）は使いません。終了時に、どのリスクが確認できて何が**未確認のまま残ったか**を
残存リスクとして報告します。
...
実行は `/explore` に `charter=` を渡して行います（チャーター駆動モード）。
チャーターの実行順・深さに従い、終了時に**残存リスク**を報告します。
