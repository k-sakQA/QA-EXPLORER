...
## 障害解析の鉄則 (必須) — 「最後に出たエラー」を原因と即断しない

テスト失敗時、特に「環境要因」「ブラウザクラッシュ」「〇〇が不安定」のように**自分の管理外の
何かのせい**にする結論を`findings.md`に書く前に、必ず以下の因果チェーンを遡って確認する。
ログの一番下に出ているエラーは時系列で最後に表面化した**症状**であることが多く、根本原因とは
限らない。

1. **最初に発生した異常は何か**: タイムスタンプ・行番号で特定する。「テストがタイムアウトした」
   「最初のアサーションが失敗した」等、時系列上もっとも早い異常を探す。
2. **その直前に何を待っていた/何を操作しようとしていたか**: 待機していたセレクタ・要素・
   リクエストを具体的に特定する。存在しない/一致しないセレクタを規定タイムアウトいっぱいまで
   待ち続けていないか(`.catch(() => {})`で握りつぶした待機の合計が積み重なっていないか)。
3. **主張したい原因(クラッシュ・環境不安定・製品バグ等)を裏付ける独立した証拠があるか**:
   ブラウザプロセスの異常終了ログ、ネットワークエラー、他のテスト・過去セッションでの再現性など。
   「後から出た二次的なエラーメッセージの文言」だけを証拠にしない。
4. 上記3点を`findings.md`に書き出してから結論を出す。独立した証拠が薄い場合は
   `Needs-Human: Yes`にして人間の判断を仰ぐか、「原因未特定、要再調査」と正直に書く。
   自テストコードの不備(セレクタ誤り等)である可能性を常に最初に疑う。

**具体例 (2026-09-02, development-pocket-heroes-net F-20260902-02)**: 実購入テストが
3回連続で `Target page, context or browser has been closed` で失敗し、当初「Stripe iframe
操作時のブラウザクラッシュ」という環境要因に分類した。しかし遡ると、最初の異常は
`Test timeout of 60000ms exceeded` であり、その直前に待っていたのは**存在しないセレクタ
(`iframe[title*="card" i]`)で永遠にマッチしないframeLocator**への3項目分のfill待機
(各`.catch(() => {})`で握りつぶされ積み重なっていた)だった。ブラウザクラッシュを示す
独立した証拠(プロセスログ等)は一切なく、「context/browser has been closed」は
Playwrightがタイムアウト後にテストを強制終了した**結果**に過ぎなかった。実際は
テストコードのセレクタ不備が根本原因だった。
...
- **チャーター駆動** (`/explore url=... charter=<path>`) で探索した場合:
  - 各イテレーションで `test-charters.md` の「状態」を
    `未実行` → `実行中` → `完了` / `未確認のまま終了` と更新する
  - `完了` の判断基準は**チャーターの完了条件**であって、テストの成否ではない。
    落ちてバグを起票できたなら `完了`
  - `coverage.md` の「リスクID」列に、その観点を消化したチャーターの `R-NN` を書く
  - セッション終了時、`未確認のまま終了` と `未実行` を**残存リスクとして報告する**
    (紐づくリスクID・深さ・理由つき)。「全部やりました」と読める報告をしない
  - チャーターに無い観点には手を出さない。順序・深さ・ケース数を自分で変えない
